...
export const usersService = {
  ...
  createManager: (ownerId: string, payload: UserDTO) =>
    apiClient
      .post<BackendApiResponse<User>>(`/api/v1/users/${ownerId}/managers`, { ...payload, role: 'MANAGER' })
      .then(unwrap),
  ...
  listManagers: async (ownerId?: string) => {
    if (!ownerId) return [];
    return apiClient.get<BackendApiResponse<User[]>>(`/api/v1/users/${ownerId}/managers`).then(unwrap);
  },
