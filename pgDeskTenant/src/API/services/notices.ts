...
export const noticesService = {
  search: (payload: NoticeBoardSearchRequest) =>
    apiClient.post<BackendApiResponse<NoticeBoardSearchResponse>>('/api/v1/notices/search', payload).then(unwrap),
  getById: (id: number) => apiClient.get<BackendApiResponse<NoticeBoard>>(`/api/v1/notices/${id}`).then(unwrap),
  ...
  archive: (id: number) => apiClient.delete<BackendApiResponse<NoticeBoard>>(`/api/v1/notices/${id}`).then(unwrap),
};
